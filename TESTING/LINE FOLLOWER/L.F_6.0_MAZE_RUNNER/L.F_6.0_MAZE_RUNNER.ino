...
{ 
  ...
  sensor[1]=digitalRead(A5);
  sensor[2]=digitalRead(A4);
  sensor[3]=digitalRead(A3);
  sensor[4]=digitalRead(A2);
  sensor[5]=digitalRead(A1);
...
void motor_control()
{
  ...
  {  
    Serial.println("I m in 11 00 00");    
    halt();
    forward();
    halt();
    l=check_l_jn();
    ...
    Serial.println("I m in 11 10 00");  
    halt();
    forward();
    halt();
    l=check_l_jn();
...
/*     else if((sensor[0]==1)&&(sensor[1]==0)&&(sensor[2]==0)&&(sensor[3]==0)&&(sensor[4]==0)&&(sensor[5]==0)) // 1 0 0 0 0 0
    {
    Serial.println("i m in 10 00 00");
    halt();
    forward();
    halt();   
    right_turn();
    ...
    else if((sensor[0]==1)&&(sensor[1]==0)&&(sensor[2]==0)&&(sensor[3]==0)&&(sensor[4]==0)&&(sensor[5]==1))  // 1 0 0 0 0 1
    {
      forward();
      halt();
      t=check_t_jn();
...
void right_turn()
{ 
  Serial.println("I m in RIGHT TURN");
  read_sensor_values();
  ...
  void left_turn()
  {
    Serial.println("I m in left Turn");
    read_sensor_values();
     while((sensor[3]==1)||(sensor[2]==1))    //3 is 1 and 2 is 1
     {
      read_sensor_values();
     digitalWrite(2, LOW);
     digitalWrite(4, HIGH);
